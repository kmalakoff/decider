...
export default class Store {
  ...
  async fetch() {
    try {
      const res = await fetch(`${process.env.API_SERVICE_URL}/query/v1/proposals`);
      this.proposals = await res.json();
    } catch (err) { alert(err); }

    try {
      const res = await fetch(`${process.env.API_SERVICE_URL}/query/v1/users`);
      this.users = await res.json();
    } catch (err) { alert(err); }

    try {
      const res = await fetch(`${process.env.API_SERVICE_URL}/query/v1/votes`);
