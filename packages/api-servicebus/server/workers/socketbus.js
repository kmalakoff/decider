...
class Socketbus {
  ...
  addSubscription(spark, { channel, query }) {
    ...
    let sparkSubscriptions = subscriptions[spark.id];
    if (!sparkSubscriptions) sparkSubscriptions = subscriptions[spark.id] = { spark, query: query || {} };
    console.log('Added subscriptions:', spark.id, channel, query);
  }

  removeSubscriptions(spark) {
    for (const channel in this.subscriptions) delete this.subscriptions[channel][spark.id];
  ...
  broadcastMessage(data) {
    ...
    if (subscriptions) {
      for (const spark_id in subscriptions) {
